...
{
	vector<pair<string,string> > userAndItem;
	vector<pair<string,string> > userAndRating;
	vector<pair<string,string> > itemAndRating;
	fstream file ("ratings_Musical_Instruments.csv");
	string value;
	while (file.good())
	{
     	getline (file, value, ',');
		...
		string rating = value;
		getline (file, value, ',');
		//cout<<userID<<endl<<itemID<<endl<<rating<<endl;
		
		userAndItem.push_back(make_pair(userID,itemID));
		userAndRating.push_back(make_pair(userID,rating));
		itemAndRating.push_back(make_pair(itemID,rating));
	}
/*
